					...
					<div class="col-sm-12">
						<div class="box">
							...
							<%@taglib uri="http://java.sun.com/jstl/core_rt" prefix="c"%>					
								<c:forEach items="${sessionScope.editassign_area }" var="y">
							
							
								<form action="<%=request.getContextPath()%>/C_Assign_Area_Controller?flag=update" method="post" class='form-horizontal form-validate' id="bb">
									...
									<div class="form-group">
										...
										<select name="staff" id="bbb"  class="form-control">
											...
											<c:forEach items="${sessionScope.staff}" var="x">
												<c:if test="${y.stf.staffId eq x.staffId}"> 
												<option value="${x.staffId}"  selected="selected">${x.staffName }</option>
												 </c:if> 
													<c:if test="${y.stf.staffId ne x.staffId}">
													<option value="${x.staffId }">${x.staffName }</option>
													</c:if>									
											</c:forEach>
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" name="van_no" id="textfield" class="form-control" data-rule-required="true" data-rule-minlength="2" value="${y.vanNo}">
										</div>
									</div>								
									<input type="hidden" name="id" value="${y.assignaId}">
