				...
				<div class="row">
					<div class="col-sm-12">
						<div class="box box-bordered box-color">
							<div class="box-title">
								<h3>
									<i class="fa fa-th-list"></i>Manage Subcategory</h3>
							</div>
							<div class="box-content nopadding">
						<%@taglib uri="http://java.sun.com/jstl/core_rt" prefix="c"%>					
						<c:forEach items="${sessionScope.editsubcat }" var="y">
								<form action="<%=request.getContextPath()%>/Subcat_Controller" method="POST" class='form-horizontal form-bordered'>
									
									<div class="form-group">
										<label for="textfield" class="control-label col-sm-2">Category Name</label>
										
										<div class="col-sm-10">
											<select name="cat_id" id="bbb" data-rule-required="true" class="form-control">
												<c:forEach items="${sessionScope.category}" var="x">
												<c:if test="${y.mc.categoryId eq x.categoryId}">
												<option value="${x.categoryId }" selected="selected">${x.categoryName }</option>
												</c:if>
													<c:if test="${y.mc.categoryId ne x.categoryId}">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" name="subcat" id="textfield" placeholder="Text input" class="form-control" value="${y.subcatName}" >
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<textarea name="subcat_des" id="textarea" rows="5" placeholder="Write Your Text Here..." class="form-control">${y.subcatDes}</textarea>
										</div>
									</div>
										<input type="hidden" name="id" value="${y.subcatId}">
